...
// Log-a-premium-payment modal for one vehicle policy. Purely additive — does
// not touch the policy's own dates. Can carry multiple attachments (receipt
// PDF, cheque scan, payment screenshot) picked from the file system or
// captured live with the phone camera; they upload together with the payment
// in one request.
export default function VehiclePremiumForm({ vehicleId, policyId, defaultAmount, defaults = {}, onClose, onSaved }) {
  ...
  const submit = async (e) => {
    ...
    setError(null);
    try {
      const saved = await api.addVehiclePremium(
        vehicleId, policyId,
        { paidOn, amount, notes, paymentMode, referenceNo, bankName },
        files
      );
      onSaved(saved);
    } catch (err) {
        ...
        <form onSubmit={submit}>
          <div className="form-grid">
            ...
            <label className="field">Payment Mode
              <input value={paymentMode} onChange={e => setPaymentMode(e.target.value)} placeholder="e.g. Auto Debit" />
            </label>
